...
namespace usbportcontrol
{
    public partial class Form1 : Form
    {
        ...
        private void EnableAllUSBPorts()
        {
            try
            {
                RunAsAdminOnCMD(devconPath + " rescan"); // USB gücünü etkinleştir
                RunAsAdminOnCMD(devconPath + " enable USB\\*"); // USB bağlantı noktalarını etkinleştir
                RunAsAdminOnCMD(devconPath + " enable USBSTOR\\*"); // USB depolama aygıtlarını etkinleştir
                RunAsAdminOnCMD(devconPath + " enable remove usb\\*");
                MessageBox.Show("Tüm USB bağlantı noktaları etkinleştirildi.");
            }
            catch (Exception ex)
            {
                MessageBox.Show(null, "Hata Oluştu: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void DisableAllUSBPorts()
        {
            try
            {
                ...
                MessageBox.Show("Tüm USB bağlantı noktaları devre dışı bırakıldı.");
            }
            catch (Exception ex)
            {
                MessageBox.Show(null, "Hata Oluştu: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void DisableUSBStorageDevices()
        {
            try
            {
                ...
                MessageBox.Show("USB depolama aygıtları devre dışı bırakıldı.");
            }
            catch (Exception ex)
            {
                MessageBox.Show(null, "Hata Oluştu: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void RemoveAllUSBDevices()
        {
            try
            {
                ...
                MessageBox.Show("Tüm USB aygıtları kaldırıldı.Açmak için butona basın ve bilgisayarınızı yeniden başlatın");
            }
            catch (Exception ex)
            {
                MessageBox.Show(null, "Hata Oluştu: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void notifcy_button_Click(object sender, EventArgs e)
        {
            ...
            notifyIcon.BalloonTipTitle = "usb port ";
            notifyIcon.BalloonTipText = "kullanmak istediğiniz butona tıklayın" ;
